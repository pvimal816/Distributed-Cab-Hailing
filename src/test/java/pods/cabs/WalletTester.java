...
public class WalletTester {
    ...
    public void getBalanceTest() {
        ...
        probe.expectMessage(new Wallet.GetBalanceResponse(1000));
        testKit.stop(wallet);
    }
    ...
    public void addBalanceTest(){
        ActorRef<Wallet.WalletCommand> wallet = testKit.spawn(Wallet.create(101, 1000), "test_wallet");
        TestProbe<Wallet.ResponseBalance> probe = testKit.createTestProbe();
        wallet.tell(new Wallet.AddBalance(100, probe.ref()));
        probe.expectMessage(new Wallet.AddBalanceResponse(true));
        wallet.tell(new Wallet.GetBalance(probe.ref()));
        probe.expectMessage(new Wallet.GetBalanceResponse(1100));
        testKit.stop(wallet);
    }
    ...
    public void deductBalanceTest(){
        ...
        probe.expectMessage(new Wallet.DeductBalanceResponse(true));
        wallet.tell(new Wallet.GetBalance(probe.ref()));
        probe.expectMessage(new Wallet.GetBalanceResponse(900));
        testKit.stop(wallet);
    }
